...
class GridWorldMDP:
    ...
    def validate_state(self, s):
        i = max(0, min(s[0], self.size - 1))
        j = max(0, min(s[1], self.size - 1))
    ...
    while True:
        ...
        for state in mdp.state_space:
            ...
            state_values[state] = max(
                sum(prob * (mdp.rewards[next_state] + gamma * state_values[next_state])
    ...
    while True:
        ...
        while True:
            ...
            for state in mdp.state_space:
                ...
                state_values[state] = sum(
                    prob * (mdp.rewards[next_state] + gamma * state_values[next_state])
                    for prob, next_state in mdp.transitions[state][action]
                )
                delta = max(delta, abs(v - state_values[state]))
            if delta < 0.01:
        ...
        for state in mdp.state_space:
            ...
            policy[state] = max(
                ...
                key=lambda a: sum(
                    prob * (mdp.rewards[next_state] + gamma * state_values[next_state])
                    for prob, next_state in mdp.transitions[state][a]
                )
            )
